...
export default function LanguageSwitcher({ className = "" }) {
  ...
  const hasTwoLocales = LOCALES.length === 2;
  ...
  return (
    <div className={className}>
      {hasTwoLocales ? (
        <div
          className="inline-flex items-center rounded-lg border border-black/10 dark:border-white/10 bg-surface p-1"
          title="Language"
          data-i18n-skip="true"
        >
          {LOCALES.map((item) => {
            ...
            return (
              ...
              >
                <span>{info.flag}</span>
                <span className="uppercase">{item.split("-")[0]}</span>
              </button>
            );
          })}
        </div>
      ...
      {!hasTwoLocales && isOpen && createPortal(
