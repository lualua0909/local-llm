...
function checkCertInstalledWindows(certPath) {
  return new Promise((resolve) => {
    ...
    exec("certutil -store Root \"localllm MITM Root CA\"", { windowsHide: true }, (error) => {
      resolve(!error);
    });
  });
}
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "localllm MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
async function uninstallCertWindows() {
  ...
  return new Promise((resolve, reject) => {
    exec(
      `certutil -delstore Root "localllm MITM Root CA"`,
      { windowsHide: true },
      (error) => {
...
function checkCertInstalledLinux() {
  const certFile = `${LINUX_CERT_DIR}/localllm-root-ca.crt`;
...
async function installCertLinux(sudoPassword, certPath) {
  ...
  const destFile = `${LINUX_CERT_DIR}/localllm-root-ca.crt`;
