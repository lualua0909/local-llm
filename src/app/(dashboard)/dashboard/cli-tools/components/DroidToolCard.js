...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id === "custom:localllm-0");
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const customModel = droidStatus.settings?.customModels?.find(m => m.id === "custom:localllm-0");
      if (customModel) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL */}
                {droidStatus?.settings?.customModels?.find(m => m.id === "custom:localllm-0")?.baseUrl && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.haslocalllm} loading={restoring}>
