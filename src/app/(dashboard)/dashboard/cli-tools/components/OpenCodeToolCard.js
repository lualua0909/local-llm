...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("localllm/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("localllm/", ""));
    }
  }, [status]);
  ...
  const getConfigStatus = () => {
    ...
    const url = status.config?.provider?.["localllm"]?.options?.baseURL || "";
    const isLocal = url.includes("localhost") || url.includes("127.0.0.1");
    return status.haslocalllm && (isLocal || url.includes(baseUrl)) ? "configured" : status.haslocalllm ? "other" : "not_configured";
  };
  ...
  const handleApply = async () => {
    ...
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_localllm" : selectedApiKey);
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_localllm" : "<API_KEY_FROM_DASHBOARD>");
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `localllm/${activeModelToShow}`,
        agent: {
          explorer: {
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current base URL */}
                {status?.config?.provider?.["localllm"]?.options?.baseURL && (
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {apiKeys.length > 0 ? (
                    ...
                    <span className="flex-1 text-xs text-text-muted px-2 py-1.5">
                      {cloudEnabled ? "No API keys - Create one in Keys page" : "sk_localllm (default)"}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.haslocalllm} loading={restoring}>
