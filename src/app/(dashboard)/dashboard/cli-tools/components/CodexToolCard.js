...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_localllm for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# localllm Configuration for Codex CLI
model = "${selectedModel}"
          ...
          {!checkingCodex && codexStatus?.installed && (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!codexStatus.haslocalllm} loading={restoring}>
