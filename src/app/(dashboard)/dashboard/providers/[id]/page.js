...
function TestProviderRow({ label, children }) {
  return (
...
function TestProviderCard({ providerId, providerStorageAlias, preferredModelId }) {
  ...
  const modelOptions = useMemo(() => {
    ...
    return [{ id: preferredModelId, name: preferredModelId }, ...models];
  ...
  useEffect(() => {
    if (preferredModelId) {
      setSelectedModel(preferredModelId);
    }
  }, [preferredModelId]);
  ...
  const modelFull = selectedModel ? `${providerStorageAlias}/${selectedModel}` : "";
  ...
  return (
    <Card>
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <TestProviderRow label="Endpoint">
...
export default function ProviderDetailPage() {
  ...
  const [lastCopiedModelId, setLastCopiedModelId] = useState("");
  ...
  const handleCopy = useCallback((text, id = "default") => {
    ...
    for (const prefix of prefixes) {
      if (typeof text === "string" && text.startsWith(prefix)) {
        const modelId = text.slice(prefix.length).trim();
        if (modelId) {
          setLastCopiedModelId(modelId);
        }
        break;
      }
    }
  }, [copy, providerDisplayAlias, providerStorageAlias, providerId]);
    ...
    <div className="flex flex-col gap-8">
      ...
      {providerId === "opencode" && (
        <TestProviderCard
          ...
          providerStorageAlias={providerStorageAlias}
          preferredModelId={lastCopiedModelId}
        />
      )}
