...
const haslocalllmConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "localllm");
};

const getlocalllmEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "localllm") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getlocalllmEntry(config);
...
// POST - Apply localllm config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing localllm entry or append
    const idx = config.findIndex((e) => e.name === "localllm");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove localllm entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "localllm");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "localllm removed from Copilot config",
    });
  } catch (error) {
