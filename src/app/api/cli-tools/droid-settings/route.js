...
// Check if settings has localllm customModels
const haslocalllmConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id === "custom:localllm-0");
};
...
// POST - Update localllm customModels (merge with existing settings)
export async function POST(request) {
  try {
    ...
    // Remove existing localllm config if any
    settings.customModels = settings.customModels.filter(m => m.id !== "custom:localllm-0");

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Add new localllm config
    const customModel = {
      model: model,
...
// DELETE - Remove localllm customModels only (keep other settings)
