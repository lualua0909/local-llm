...
const haslocalllmConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["localllm"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["localllm"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      haslocalllm: haslocalllmConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("localllm/") ? config.model.replace(/^localllm\//, "") : null,
...
// POST - Apply localllm as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_localllm";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing localllm provider entry and its models
    const existingProvider = config.provider["localllm"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["localllm"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `localllm/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `localllm/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("localllm/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove localllm provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["localllm"]?.models) {
      delete config.provider["localllm"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["localllm"].models).length === 0) {
        delete config.provider["localllm"];
        if (config.model?.startsWith("localllm/")) delete config.model;
      } else if (config.model === `localllm/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["localllm"].models);
        config.model = `localllm/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire localllm provider
      if (config.provider) delete config.provider["localllm"];
      if (config.model?.startsWith("localllm/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("localllm/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "localllm settings removed from OpenCode",
    });
  } catch (error) {
