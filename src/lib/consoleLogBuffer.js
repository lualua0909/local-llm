...
export function appendConsoleLog(tag, payload) {
  const safeTag = (typeof tag === "string" && tag.trim()) ? tag.trim().toUpperCase() : "LOG";
  const message = payload === undefined ? "" : formatArg(payload);
  const line = message
    ? `[${formatTime()}] [${safeTag}] ${message}`
    : `[${formatTime()}] [${safeTag}]`;
  appendLine(line);
}
