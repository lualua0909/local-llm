# localllm Architecture

_Last updated: 2026-02-06_

## Executive Summary

localllm is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[localllm Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.localllm/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.localllm/usage.json`, `~/.localllm/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.localllm/db.json`)
- usage stats: `~/.localllm/usage.json`
- request log lines: `~/.localllm/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[localllm Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.localllm` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/localllm && npm run build`
