...
import { appendConsoleLog } from "@/lib/consoleLogBuffer.js";
import { compactContext } from "../services/contextCompaction.js";
import { sanitizePromptInputs } from "../security/promptSanitizer.js";

function pickPromptFields(payload) {
...
function emitPromptLog({ provider, model, sourceFormat, targetFormat, passthrough, body, translatedBody }) {
  try {
    appendConsoleLog("PROMPT", {
      provider,
      model,
      sourceFormat,
      targetFormat,
      passthrough,
      original: pickPromptFields(body),
      translated: pickPromptFields(translatedBody)
    });
  } catch {
    // Prompt logging should never block request execution
  }
}
 ...
 */
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, sourceFormatOverride, contextCompactionPolicy }) {
  const { provider, model } = modelInfo;
  const requestStartTime = Date.now();
  body = sanitizePromptInputs(body);
  ...
  const compacted = compactContext(body, sourceFormat, contextCompactionPolicy);
  if (compacted.changed) {
    body = compacted.body;
    log?.info?.("COMPACT", `${provider.toUpperCase()} | ${model} | ${compacted.stats.beforeMessages} -> ${compacted.stats.afterMessages} | ~${compacted.stats.estimatedSavedTokens} tokens saved`);
    appendConsoleLog("PROMPT", {
      type: "compaction",
      provider,
      model,
