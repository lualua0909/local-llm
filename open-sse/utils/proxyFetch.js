...
function parseAllowedHosts() {
  const raw = (process.env.OUTBOUND_ALLOWED_HOSTS || "").trim();
  if (!raw) return null;
...
function validateOutboundTarget(rawUrl) {
  let parsed;
  try {
    parsed = new URL(rawUrl);
  } catch {
    throw new Error(`[ProxyFetch] Invalid outbound URL: ${rawUrl}`);
  }

  if (!["http:", "https:"].includes(parsed.protocol)) {
    throw new Error(`[ProxyFetch] Blocked non-HTTP(S) outbound URL: ${parsed.protocol}`);
  }

  if (parsed.username || parsed.password) {
    throw new Error("[ProxyFetch] Blocked outbound URL with embedded credentials");
  }

  const hostname = parsed.hostname.toLowerCase();
  const allowPrivate = process.env.ALLOW_PRIVATE_OUTBOUND !== "false";
  if (!allowPrivate && isPrivateHost(hostname)) {
    throw new Error(`[ProxyFetch] Blocked private-network outbound host: ${hostname}`);
  }

  const allowedHosts = parseAllowedHosts();
  if (!hostIsAllowed(hostname, allowedHosts)) {
    throw new Error(`[ProxyFetch] Host not in OUTBOUND_ALLOWED_HOSTS: ${hostname}`);
  }
...
export async function proxyAwareFetch(url, options = {}, proxyOptions = null) {
  ...
  const parsedTarget = validateOutboundTarget(targetUrl);
  const method = (options?.method || "GET").toUpperCase();
  console.log(`[OUTBOUND] ${method} ${parsedTarget.origin}${parsedTarget.pathname}`);
